 ...
 */
<template>
  <div :style="styleVar" class="M-Flipper" :class="[flipType, {'go': isFlipping}]">
...
export default {
  ...
  methods: {
    _textClass(number:any) {
      return 'number' + number
    },
    _flip(type:any, front:any, back:any) {
      // 如果处于翻转中，则不执行
      if (this.isFlipping) {
        return false
      }
    ...
    // 下翻牌
    flipDown(front: any, back: any) {
      this._flip('down', front, back)
    },
    // 上翻牌
    flipUp(front: any, back: any) {
  ...
  created() {
      this.frontTextFromData = this.frontText as any
      this.backTextFromData = this.backText as any
  },
  computed: {
      styleVar() {
          return {
...
.M-Flipper {
  ...
  /* border: solid 1px var(--clock-bgcolor); */
  border-radius: 10px;
  /* background: #fff; */
  font-size: calc(var(--clock-size)*0.7);
  color: #fff;
  box-shadow: 0 0 6px var(--clock-bgcolor);
  text-align: center;
